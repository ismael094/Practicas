...
class HeaderDAO(object):
    ...
    def __init__(self, cursor):
        ...
        self.sentence = ("insert into header "
            "(id_frame, id_header_definition, order_keyword, extension, string_value, long_value, double_value) "
    ...
    def save(self, header, headerDefinition):
        try: 
            ...
            if header.type == 'string_value':
                ...
                double = header.value
            values = (header.idFrame, headerDefinition.id, header.orderKeyword, header.extension, string, longVal, double)
            self.cursor.execute(self.sentence, values)
...
class HeaderPDAO(object):
    ...
    def setDAO(self, header, headerDefinition):
        ...
        self.sentence2Data.append((int(header.idFrame),int(headerDefinition.id), int(header.orderKeyword), int(header.extension), str(string), longVal, double)) 
        logging.debug('%s = %s' % (headerDefinition.name, header.value))
    ...
    def save(self):
        try: 
            ...
            sql = "insert into header(id_frame,order_keyword,extension, string_value,long_value,\
            double_value) values (%s, %s, %s, '%s' , %s, %s)" % (self.sentence2Data)
            return False
...
def getDataFitsImages(path):
    try:
        ...
        logging.warning('Dump has been cancel')
...
class DataBasePopulator(object):
    ...
    def headerPopulator(self, extension, keyword):
        ...
        headerList = [extension, keywordInsertValue, value, position, self.models.headerDefinition.id]
