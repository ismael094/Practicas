    ...
    for key in keyword:
        try:
            ...
            if key == 'INSTRUME' or key == 'OBSMODE' or key == 'DATE':
                data[0][key] = [getDataByPath(key, path), None]
            else:
                data[0][key] = ['',None]
...
def startScript(DAOs,args):
    route = args.route
    scan = args.scan
    default = args.default
    filepath, fileextension = os.path.splitext(route)
    if scan == True and fileextension == '': 
        start = fileScaner(DAOs,default, route)
    elif scan != True and fileextension != '':
        start = startDumpProcess(DAOs,route)

def fileScaner(DAOs,pathRoot,path1):
    path = pathRoot+path1
    yu = glob.glob(path)
    for dire in yu:
        for root, dirs, files in os.walk(dire):
            for fil in files:
                if fil.endswith(".fits"):
                    ...
                    Open = startDumpProcess(DAOs,final_root)
        DAOs.db.commit()

def startDumpProcess(DAOs,path):
    data = getDataFitsImages(path)
    checkKeyword(data, path)
    dump = dataBasePopulator(DAOs,data, path)
    ...
    for extension in range(len(image)):
        for keyword in image[extension].header:
            if keyword != 'COMMENT':
                ...
                image[extension].header.comments[keyword], extension]
    image.close()
    return data

def dataBasePopulator(DAOs,data,path):
    checkProgramKey(data, path)
    camera = Camera()
    setCamera(camera, data)
    DAOs.cameraDAO.getId(camera)
    observationMode = ObservationMode()
    setObservationMode(observationMode, data, camera)
    if not DAOs.observationModeDAO.getId(observationMode):
        DAOs.observationModeDAO.save(observationMode)
    frameData = Frame()
    setFrame(frameData, data, path, camera.id, observationMode.id)
    DAOs.frameDAO.save(frameData)
    headerDefinition = HeaderDefinition()
    header = Header()
    for keyword in data:
        value = data[keyword][0]
        observationModeId = frameData.id_observation_mode
        keywordType = getKeywordType(value)
        headerDefList = [data[keyword][1],keyword,\
        keywordType,observationModeId]
        setHeaderDefinition(headerDefinition,headerDefList)
        if DAOs.headerDefinitionDAO.getId(headerDefinition) == False:
            DAOs.headerDefinitionDAO.save(headerDefinition)
        keywordInsertValue = getInsertValue(value)
        headerList = [headerDefinition.id, frameData.id, data[keyword][2],\
        keywordInsertValue, value]
        setHeader(header,headerList)
        headerInsert = DAOs.headerDAO.save(header)
